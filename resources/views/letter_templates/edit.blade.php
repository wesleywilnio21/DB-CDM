<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Letter Template') }}: {{ $letterTemplate->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <form action="{{ route('letter-templates.update', $letterTemplate) }}" method="POST" id="template-form" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" value="{{ old('name', $letterTemplate->name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                        </div>
                        
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700">Number Format (e.g. {nomor}/CDM/{bulan_romawi}/{tahun})</label>
                            <input type="text" name="number_format" value="{{ old('number_format', $letterTemplate->number_format) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm font-mono text-sm" required>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4 pt-4 border-t">
                        <div>
                            ...
                            <input type="text" name="signatory_name" value="{{ old('signatory_name', $letterTemplate->signatory_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Signatory Position</label>
                            <input type="text" name="signatory_position" value="{{ old('signatory_position', $letterTemplate->signatory_position) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        </div>
                        <div>
                            ...
                            @if($letterTemplate->signature_image)
                                <div class="mt-2 text-xs text-gray-500">Current: <img src="{{ Storage::url($letterTemplate->signature_image) }}" class="h-12 object-contain hidden" onload="this.classList.remove('hidden')"></div>
                            @endif
                        </div>
                        <div>
                            ...
                            @if($letterTemplate->stamp_image)
                                <div class="mt-2 text-xs text-gray-500">Current: <img src="{{ Storage::url($letterTemplate->stamp_image) }}" class="h-12 object-contain hidden" onload="this.classList.remove('hidden')"></div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-2 mt-6 border-t pt-4">
                        ...
                        <button type="submit" class="bg-gray-900 text-white px-4 py-2 rounded-md shadow hover:bg-gray-800">Update Template</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        var rawContent = `{!! addslashes(old('content', $letterTemplate->content)) !!}`;
