    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <form action="{{ route('letter-templates.update', $letterTemplate) }}" method="POST" class="p-6 md:p-8 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="title" value="{{ old('title', $letterTemplate->title) }}" required class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="bg-white text-gray-900 rounded-xl overflow-hidden border border-gray-200 focus-within:border-gray-400 transition-colors">
                                <div id="quill-editor" style="height: 300px;">{!! old('body', $letterTemplate->body) !!}</div>
                            </div>
                            <input type="hidden" name="body" id="body-input" value="{{ old('body', $letterTemplate->body) }}">
                            ...
                            <div class="bg-gray-50 p-6 rounded-2xl border border-gray-100 space-y-6">
                                ...
                                <div>
                                    ...
                                    <select name="logo_asset_id" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900 bg-white">
                                        ...
                                        @foreach($logos as $logo)
                                            <option value="{{ $logo->id }}" {{ old('logo_asset_id', $letterTemplate->logo_asset_id) == $logo->id ? 'selected' : '' }}>{{ $logo->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="md:col-span-2">
                                        ...
                                        <input type="text" name="sig_text_above" value="{{ old('sig_text_above', $letterTemplate->sig_text_above) }}" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900 bg-white" />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select name="kop_asset_id" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900 bg-white">
                                            ...
                                            @foreach($kops as $kop)
                                                <option value="{{ $kop->id }}" {{ old('kop_asset_id', $letterTemplate->kop_asset_id) == $kop->id ? 'selected' : '' }}>{{ $kop->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select name="ttd_asset_id" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900 bg-white">
                                            ...
                                            @foreach($ttds as $ttd)
                                                <option value="{{ $ttd->id }}" {{ old('ttd_asset_id', $letterTemplate->ttd_asset_id) == $ttd->id ? 'selected' : '' }}>{{ $ttd->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="sig_name" value="{{ old('sig_name', $letterTemplate->sig_name) }}" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900 bg-white" />
                                    </div>

                                    <div>
                                        <label class="block text-sm font-semibold text-gray-900 mb-1">Signer Position</label>
                                        <input type="text" name="sig_position" value="{{ old('sig_position', $letterTemplate->sig_position) }}" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900 bg-white" />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="pt-6 border-t border-gray-100 flex justify-end gap-3">
                        ...
                        <button type="submit" onclick="document.getElementById('body-input').value = quill.root.innerHTML" class="px-5 py-2.5 bg-gray-900 border border-transparent rounded-full font-medium text-sm text-white hover:bg-gray-800 focus:outline-none transition-all shadow-sm">Update Template</button>
                    </div>
                </form>
            </div>
        </div>
