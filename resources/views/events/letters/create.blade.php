    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <form action="{{ route('letters.store', $event) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 space-y-6">
                    @csrf
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-gray-900 mb-1">Letter Title <span class="text-red-500">*</span></label>
                            <input type="text" name="title" value="{{ old('title') }}" required class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900" placeholder="e.g., Invitation Letter" />
                            <x-input-error :messages="$errors->get('title')" class="mt-1" />
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-900 mb-1">Event Code</label>
                            <input type="text" name="event_code" value="{{ old('event_code') }}" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900" placeholder="Auto-generated if empty" />
                            <p class="text-xs text-gray-500 mt-1">Leave empty to use default. Preview: {{ $previewLetterNumber }}</p>
                            <x-input-error :messages="$errors->get('event_code')" class="mt-1" />
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-900 mb-1">Issued Date <span class="text-red-500">*</span></label>
                            <input type="date" name="issued_at" value="{{ old('issued_at', now()->format('Y-m-d')) }}" required class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900" />
                        ...
                        <div>
                            ...
                            <input type="text" name="city" value="{{ old('city', $defaultCity) }}" required class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900" />
                            <x-input-error :messages="$errors->get('city')" class="mt-1" />
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-900 mb-1">Recipient Name <span class="text-red-500">*</span></label>
                            <input type="text" name="recipient_name" value="{{ old('recipient_name') }}" required class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900" placeholder="John Doe" />
                        ...
                        <div>
                            ...
                            <input type="text" name="recipient_phone" value="{{ old('recipient_phone') }}" class="block w-full border-gray-200 focus:border-gray-400 focus:ring-0 rounded-xl shadow-sm text-sm text-gray-900" placeholder="+1234567890" />
                    ...
                    <div class="pt-6 border-t border-gray-100 flex justify-end gap-3">
                        ...
                        <button type="submit" onclick="document.getElementById('body-input').value = quill.root.innerHTML" class="px-5 py-2.5 bg-gray-900 border border-transparent rounded-full font-medium text-sm text-white hover:bg-gray-800 focus:outline-none transition-all shadow-sm">Save Letter</button>
                    </div>
                </form>
            </div>
        </div>
