    ...
    <div class="py-10" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div
                    class="p-6 md:p-8 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <h3 class="text-xl font-bold text-gray-900 tracking-tight">System Users</h3>
                    <button @click="showCreate = true"
                        class="inline-flex items-center justify-center px-5 py-2.5 bg-gray-900 border border-transparent rounded-full font-bold text-sm text-white hover:bg-black transition-all shadow-md">
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div
                                                    class="h-10 w-10 rounded-full bg-gray-900 flex items-center justify-center text-white font-bold">
                                                ...
                                                <div class="ml-4">
                                                    <div class="text-sm font-bold text-gray-900">
                                                        ...
                                                        @if($user->id === Auth::id())
                                                            <span
                                                                class="ml-2 text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full uppercase font-black">You</span>
                                                        @endif
                                                    </div>
                                            ...
                                            <div class="flex items-center justify-end gap-3">
                                                ...
                                                @if(Auth::user()->isSuperAdmin() && $user->id !== Auth::id())
                                                    <form action="{{ route('users.destroy', $user) }}" method="POST"
                                                        onsubmit="return confirm('WARNING: This action cannot be undone. Delete this user?');"
                                                        class="inline-block">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="inline-flex items-center px-3 py-1 bg-red-50 text-red-700 border border-red-200 rounded-lg hover:bg-red-100 transition-colors">
                ...
                <div class="bg-white rounded-3xl shadow-xl transform transition-all max-w-lg w-full p-8 z-10">
                    ...
                    <form method="POST" action="{{ route('users.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="role"
                                class="mt-1 block w-full border-gray-300 rounded-xl focus:border-gray-900 focus:ring-0 text-black">
                        ...
                        <div class="pt-4 flex justify-end gap-3">
                            <button type="button" @click="showCreate = false"
                                class="px-5 py-2.5 border border-gray-200 rounded-full font-bold text-gray-700">Cancel</button>
                            <button type="submit"
                                class="px-5 py-2.5 bg-gray-900 text-white rounded-full font-bold">Create User</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="bg-white rounded-3xl shadow-xl transform transition-all max-w-lg w-full p-8 z-10">
                    ...
                    <form method="POST" :action="`/users/${editUser.id}`" class="space-y-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="edit_name" name="name" type="text" class="mt-1 block w-full rounded-xl"
                        ...
                        <div>
                            ...
                            <x-text-input id="edit_username" name="username" type="text"
                                class="mt-1 block w-full rounded-xl" x-model="editUser.username" required />
                            <x-input-error :messages="$errors->get('username')" />
                        </div>
                        <div>
                            <x-input-label for="edit_email" :value="__('Email (Optional)')"
                                class="text-gray-900 font-bold" />
                            <x-text-input id="edit_email" name="email" type="email" class="mt-1 block w-full rounded-xl"
                        ...
                        <div>
                            ...
                            <select name="role" x-model="editUser.role"
                                ...
                                <option value="super_admin"
                                    :disabled="editUser.role === 'super_admin' && !{{ Auth::user()->isSuperAdmin() ? 'true' : 'false' }}">
                                    Super Admin</option>
                            </select>
                        </div>
                        <div class="pt-4 border-t border-gray-100">
                            <p class="text-xs text-gray-500 mb-2 font-bold">Leave password blank to keep current
                                password.</p>
                        ...
                        <div>
                            <x-input-label for="edit_password_confirmation" :value="__('Confirm New Password')"
                                class="text-gray-900 font-bold" />
                            <x-text-input id="edit_password_confirmation" name="password_confirmation" type="password"
                                class="mt-1 block w-full rounded-xl" />
                        </div>
                        <div class="pt-4 flex justify-end gap-3">
                            <button type="button" @click="showEdit = false"
                                class="px-5 py-2.5 border border-gray-200 rounded-full font-bold text-gray-700">Cancel</button>
                            <button type="submit"
                                class="px-5 py-2.5 bg-gray-900 text-white rounded-full font-bold">Update User</button>
                        </div>
                    </form>
                </div>
            </div>
